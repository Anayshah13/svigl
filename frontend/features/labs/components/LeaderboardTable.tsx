...
import { formatLabDate, formatLabScore } from "../format";
        ...
        <table className="w-full min-w-[28rem] border-collapse text-left">
          ...
          <tbody>
            {entries.map((entry, index) => {
              const isMe = Boolean(
                highlightUserId && entry.userId && entry.userId === highlightUserId,
              );
              return (
                <tr
                  key={`${entry.rank}-${entry.userId ?? entry.player}`}
                  className={cn(
                    "border-b border-gray-100 last:border-b-0 transition-colors hover:bg-plum-light/25",
                    index < 3 && !isMe && "bg-white",
